...
```
Запускаем Celery (**_в отдельном окне консоли_**, открытом по тому же пути):
```
celery -A task_tracker.celery worker -l info
```
**_Опционально:_** Запуск Flower (**_в отдельном окне консоли_**, открытом по тому же пути). Мониторинг задач в celery, будет доступен по http://127.0.0.1:5555
```
